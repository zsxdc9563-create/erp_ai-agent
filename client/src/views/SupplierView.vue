  ...
  <div class="module-page">
    ...
    <div class="section-card">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="s in filteredSuppliers" :key="s.id" class="table-row">
              <td class="mono text-accent">{{ s.id }}</td>
              <td>
                <div class="name-cell">
              ...
              <td>{{ s.contact }}</td>
              <td>{{ s.phone }}</td>
              <td class="text-accent">{{ s.email }}</td>
      ...
      <div class="pagination">
        <span class="page-info">共 {{ filteredSuppliers.length }} 筆</span>
      </div>
    </div>
        ...
        <div class="modal-body">
          <div class="form-grid">
            <div class="form-group">
              <label>供應商名稱 *</label>
              <input v-model="form.name" class="form-control" placeholder="輸入供應商名稱" />
            </div>
            <div class="form-group">
              <label>聯絡人</label>
...
const filteredSuppliers = computed(() => {
  ...
  return suppliers.value.filter(s =>
    ...
    s.contact.includes(searchQuery.value) ||
...
async function submitForm() {
  ...
  try {
    if (editItem.value) {
      const res = await updateSupplier(editItem.value.id, form.value)
      if (res.success) {
        const idx = suppliers.value.findIndex(s => s.id === editItem.value.id)
        if (idx !== -1) suppliers.value[idx] = res.data
      }
    } else {
      const res = await createSupplier(form.value)
      if (res.success) suppliers.value.unshift(res.data)
    }
...
async function deleteItem(item) {
  ...
  if (res.success) suppliers.value = suppliers.value.filter(s => s.id !== item.id)
}
